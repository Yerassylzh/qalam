...
import { fetchPublishedArticles } from "@/features/admin/lib/articles";
...
export default function Page() {
  ...
  useEffect(() => {
    const wrapper = async () => {
      setArticles(await fetchPublishedArticles(user.id));
    };
    wrapper();
  }, [user.id]);
  ...
  return (
    <div className="w-full flex-1 flex flex-col items-center justify-center">
      <div className="p-[10px] xl:w-[768px] md:w-[600px] w-full flex-1 flex flex-col md:gap-[40px] gap-[20px]">
        <p className="font-bold text-[48px]">Опубликованные</p>
        {articles !== null ? (
          articles.length > 0 ? (
